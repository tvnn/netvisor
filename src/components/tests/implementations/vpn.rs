        ...
        NodeTarget::Hostname(_) | NodeTarget::Service(_) => {
            ...
            match &node.base.target {
                NodeTarget::Hostname(hostname_config) => {
                    match dns_capability.resolve_domain(&hostname_config.hostname, config.timeout_ms).await {
                ...
                NodeTarget::Service(service_config) => {
                    match dns_capability.resolve_domain(&service_config.hostname, config.timeout_ms).await {
                        Ok(dns_result) if dns_result.success && !dns_result.resolved_addresses.is_empty() => {
                            Some(dns_result.resolved_addresses[0])
                        },
                        _ => None // DNS resolution failed, continue without IP
