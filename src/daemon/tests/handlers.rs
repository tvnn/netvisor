...
use std::sync::Arc;
use crate::{daemon::{runtime::types::base::DaemonState, tests::service::DaemonTestService}, server::shared::types::api::ApiError};
use crate::{
    server::{
        daemons::types::api::{
...
async fn handle_test_execution(
    ...
    let test_service = DaemonTestService::new(state.config.clone());
