...
use std::{sync::Arc};
use crate::server::{
        capabilities::{handlers as capability_handlers, types::base::{CapabilityDiscriminants}}, config::AppState, daemons::handlers as daemon_handlers, discovery::handlers as discovery_handlers, node_groups::handlers as group_handlers, nodes::{handlers as node_handlers, types::{criticality::TestCriticality, status::NodeStatus, targets::NodeTarget, types::NodeType}}, shared::types::{api::ApiResponse, metadata::{TypeMetadataProvider, TypeRegistry}}, subnets::handlers as subnet_handlers
    };
...
async fn get_type_registry() -> Json<ApiResponse<TypeRegistry>> {
    let registry = TypeRegistry {
        ...
        capabilities: CapabilityDiscriminants::iter().map(|t| t.to_metadata()).collect(),
