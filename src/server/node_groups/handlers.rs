...
use std::sync::Arc;
use crate::{
    server::shared::types::api::{ApiResult, ApiResponse, ApiError,},
    server::node_groups::{
    ...
    server::nodes::types::api::NodeListResponse,
...
async fn get_node_group(
    ...
    match service.get_group(&id).await? {
        ...
        None => Err(ApiError::not_found(&format!("Node group'{}' not found", &id))),
    }
}
