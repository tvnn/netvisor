<script lang="ts">
  ...
  onMount(() => {
    diagnosticsTableActionsStore.set({
      handleViewDetails,
      handleDelete,
      deletingId
    });
  ...
  $: diagnosticsTableActionsStore.update(current => ({ 
    ...current, 
    deletingId 
  }));
  ...
  async function handleDelete(execution: DiagnosticExecution) {
    if (confirm(`Are you sure you want to delete the diagnostic execution?`)) {
      ...
      const success = await deleteDiagnosticExecutions(execution.id);
      deletingId = null;
    }
  }
...
<div class="space-y-6">
  <!-- Header -->
   <TabHeader
    ...
    buttons={[
      {
        onclick: getDiagnosticExecutions,
