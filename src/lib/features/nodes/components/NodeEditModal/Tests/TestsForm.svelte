<script lang="ts">
  ...
  import ListManager from '$lib/components/shared/components/forms/ListManager.svelte';
  import TestConfigPanel from './TestConfigPanel.svelte';
  import { getCriticalityColor, getCriticalityDisplay, getTestDisplay, testTypes, type TypeMetadata } from "$lib/components/shared/stores/registry";
  import { getBgColor, getTextColor } from "$lib/components/shared/constants/colors";
	import { fetchCapabilityForms } from "$lib/components/capabilities/store";
  
  let tests: AssignedTest[] = [];
  ...
  async function preloadSchemas() {
    ...
    try {
      ...
      const schemas = await fetchCapabilityForms(nodeContext);
