...
use crate::server::{
    groups::{service::GroupService, types::Group},
    hosts::{service::HostService, types::base::Host},
    services::{service::ServiceService, types::base::Service},
    subnets::{
        service::SubnetService,
        types::base::{Subnet, SubnetType},
    },
    topology::types::base::{
        Edge, EdgeHandle, EdgeType, Node, NodeLayout, NodeType, SubnetChild, SubnetChildNodeSize,
        SubnetLayout, XY,
    },
};
...
impl TopologyService {
    ...
    pub async fn build_graph(&self) -> Result<Graph<Node, Edge>, Error> {
        ...
        let (subnet_sizes, child_nodes) =
            self.create_subnet_child_nodes(&hosts, &subnets, &services);
    ...
    fn calculate_container_size_and_child_positions(
        &self,
        child_sizes: Vec<(Uuid, XY)>,
        container_grid: &XY,
        padding: &XY,
    ) -> (HashMap<Uuid, XY>, XY) {
    ...
    fn calculate_subnet_size(
        ...
        let (regular_grid_size, regular_grid_dimensions, regular_child_positions) = {
            let regular_grid_dimensions =
                self.calculate_container_grid_dimensions(regular_children.len());
            let (regular_child_positions, regular_grid_size) = self
                .calculate_container_size_and_child_positions(
                    regular_children
                        .iter()
                        .map(|c| (c.id, c.size.size()))
                        .collect::<Vec<_>>(),
    ...
    fn create_subnet_nodes(
        ...
        let mut all_subnet_entries: Vec<(Uuid, &SubnetLayout)> =
            layouts.iter().map(|(id, layout)| (*id, layout)).collect();
        ...
        let container_grid_dimensions =
            self.calculate_container_grid_dimensions(all_subnet_entries.len());

        let (positions, _) = self.calculate_container_size_and_child_positions(
            all_subnet_entries
                .iter()
                .map(|(id, layout)| (*id, layout.size.clone()))
                .collect::<Vec<_>>(),
                ...
                host.base
                    ...
                    .for_each(|interface| {
                        if let (Some(&source_idx), Some(&target_idx)) = (
                            node_indices.get(&interface.id),
                            node_indices.get(&first_interface.id),
                        ) {
                            graph.add_edge(
                                source_idx,
                                target_idx,
    ...
    fn add_group_edges(
        ...
        for group in groups {
            ...
            for window in bindings.windows(2) {
                if let (Some(&source_idx), Some(&target_idx)) = (
                    node_indices.get(&window[0].interface_id),
                    node_indices.get(&window[1].interface_id),
                ) {
                    graph.add_edge(
                        source_idx,
                        target_idx,
