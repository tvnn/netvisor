...
impl GroupService {
    pub fn new(group_storage: Arc<dyn GroupStorage>, service_service: Arc<ServiceService>) -> Self {
        Self {
            group_storage,
            service_service,
        }
    ...
    pub async fn create_group(&self, group: Group) -> Result<Group> {
        ...
        for binding in &group.base.service_bindings {
            if let Some(mut service) = self
                .service_service
                .get_service(&binding.service_id)
                .await?
            {
                ...
                service.base.groups = {
                    service.base.groups.push(group.id);
                    service.base.groups
                };
    ...
    pub async fn update_group(&self, mut group: Group) -> Result<Group> {
        ...
        for old_binding in &old_group.base.service_bindings {
            if !group.base.service_bindings.contains(old_binding) {
                if let Some(mut service) = self
                    .service_service
                    .get_service(&old_binding.service_id)
                    .await?
                {
                    if !service.base.groups.contains(&group.id) {
                        continue; // Not in group
                    }
                    service.base.groups = service
                        .base
                        .groups
                        .iter()
                        .filter(|g| *g != &group.id)
        ...
        for new_binding in &group.base.service_bindings {
            if !old_group.base.service_bindings.contains(new_binding) {
                if let Some(mut service) = self
                    .service_service
                    .get_service(&new_binding.service_id)
                    .await?
                {
                    if service.base.groups.contains(&group.id) {
                        continue; // Already in group
