...
export const ipAddressInCidr = (cidr: string): Validator => (value: any) => {
  
  if (!isValidCIDR(cidr)) return {valid: false, name: `${cidr} is not valid CIDR notation`}
  if (!isValid(value)) return {valid: true, name: `IP invalid, ipAddress validator will handle`}
  if (!parse(value).match(parseCIDR(cidr))) return {valid: false, name: `IP not in range of ${cidr}`}
...
export const hostname = (): Validator => (value: string) => {
  ...
  const hostnameRegex = /^(?=.{1,255}$)(?:(?!-)[-a-zA-Z0-9]{1,63}(?:\.[a-zA-Z0-9][-a-zA-Z0-9]{0,61}[a-zA-Z0-9])?)*\.[a-zA-Z]{2,63}$/;
  return {
