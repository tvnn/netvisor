<script lang="ts">
	...
	function getCardClasses(option: RichOption, isSelected: boolean) {
		let baseClasses =
			'flex items-start space-x-3 cursor-pointer p-3 border rounded-lg transition-colors';
	...
	{#each options as option}
		...
		<label class={getCardClasses(option, selected)}>
			<input
				...
				class="mt-0.5 {mode === 'radio'
					? 'border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500'
					: 'rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500'}"
			/>
			<div class="min-w-0 flex-1">
				<div class={getTitleClasses(option, selected)}>
					...
					{#if option.category}
						<span
							class="ml-2 inline-flex items-center rounded bg-gray-600 px-2 py-0.5 text-xs font-medium text-gray-300"
						>
							{option.category}
						</span>
					{/if}
